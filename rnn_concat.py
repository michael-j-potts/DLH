...
input_seqs_icd = np.array(pickle.load(open('./MIMICIIIPROCESSED.3digitICD9.seqs', 'rb')), dtype = object)
input_seqs_meds = np.array(pickle.load(open('./MIMICIIIPROCESSED.meds.seqs', 'rb')), dtype = object)
input_seqs_labs = np.array(pickle.load(open('./MIMICIIIPROCESSED.abnlabs.seqs', 'rb')), dtype = object)
input_seqs_fullicd = np.array(pickle.load(open('./MIMICIIIPROCESSED.seqs', 'rb')), dtype = object)
...
parser.add_argument('--hosp', type=str, default='./MIMICIIIPROCESSED.morts',
                    help='location of the embedding weight file (default: ./MIMICIIIPROCESSED.morts)')
...
labels = np.array(pickle.load(open('./MIMICIIIPROCESSED.morts', 'rb')), dtype = float)

if args.hosp == './MIMICIIIPROCESSED.hospmorts':
	print("Prediction based on in hospital mortality")
	labels = np.array(pickle.load(open('./MIMICIIIPROCESSED.hospmorts', 'rb')), dtype = float)
...
print('Starting rnn_concat training..')
...
print("RNN concatenate only completed in : ", int(minutes/60), " minutes and ", seconds, "seconds.")

...
input_seqs_icd = np.array(pickle.load(open('./MIMICIIIPROCESSED.3digitICD9.seqs', 'rb')))
input_seqs_meds = np.array(pickle.load(open('./MIMICIIIPROCESSED.meds.seqs', 'rb')))
input_seqs_labs = np.array(pickle.load(open('./MIMICIIIPROCESSED.abnlabs.seqs', 'rb')))
input_seqs_fullicd = np.array(pickle.load(open('./MIMICIIIPROCESSED.seqs', 'rb')))

icditems = pickle.load(open('./MIMICIIIPROCESSED.types', 'rb'))
meditems = pickle.load(open('./MIMICIIIPROCESSED.meds.types', 'rb'))
labitems = pickle.load(open('./MIMICIIIPROCESSED.abnlabs.types', 'rb'))
...
labels = np.array(pickle.load(open('./MIMICIIIPROCESSED.morts', 'rb')))
labels2 = np.array(pickle.load(open('./MIMICIIIPROCESSED.hospmorts', 'rb')))
...
for run in range(10):
	...
	train_labels2 = rlabels2[:trainlindex]
	train_labels2 = train_labels2.reshape(train_labels2.shape[0],1)
	...
	valid_labels2 = rlabels2[trainlindex:validlindex]
	...
	test_labels2 = rlabels2[validlindex:]
	...
	for epoch in range(n_epochs):
		...
		for i in (range(0, n_iters, batchsize)):
			...
			batch_train_labels2 = train_labels2[i:i+batchsize]
			...
			for iter in range(len(batch_icd)):
				...
				targets2 = Variable(torch.from_numpy(batch_train_labels2[iter]).float())
				...
				losses2.append(criterion(outputs[[-1]], targets2))

			loss = sum(losses)/len(batch_icd)
			loss2 = sum(losses2)/len(batch_icd)
			loss.backward()
			optimizer.step()
		...
		print("Validation AUC_ROC (Total mortality): ", roc_auc_score(valid_labels, vpredictions))
		print("Validation AUC_ROC (Hospital mortality): ", roc_auc_score(valid_labels2, vpredictions))
		...
		print("Test AUC_ROC (Total mortality): ", roc_auc_score(test_labels, predictions))
		print("Test AUC_ROC (Hospital mortality): ", roc_auc_score(test_labels2, predictions))
		
		aucrocs.append(roc_auc_score(test_labels, predictions))
		aucrocs2.append(roc_auc_score(test_labels2, predictions))
	...
	best_aucrocs2.append(max(aucrocs2))

print("Average Total AUCROC:", np.mean(best_aucrocs), "+/-", np.std(best_aucrocs)) 		
print("Average Hospital AUCROC:", np.mean(best_aucrocs2), "+/-", np.std(best_aucrocs2)) 	
...
for i in (range(len(test_input_seqs_icd))):
	...
	if (test_score>0.5):
		interpretation_file.write("ID: " + str(i) + " True label (Total Mortality): "+str(test_labels[i])+"\n")
		interpretation_file.write("ID: " + str(i) + " True label (Hospital Mortality): "+str(test_labels2[i])+"\n")

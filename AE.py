...
model = AE(10,50,175)
ICD_data = pickle.load(open('./CAEEntries.3digitICD9','rb'))
Med_data = pickle.load(open('./CAEEntries.meds','rb'))
Lab_data = pickle.load(open('./CAEEntries.abnlabs','rb'))

model.fit(np.array(ICD_data), np.array(Med_data), np.array(Lab_data))

emb_weights = model._modules['emb'].weight.data.numpy().T
print('Pickled embedding weights. Shape:', np.array(emb_weights).shape)
pickle.dump(emb_weights, open('./AE_embedding_weights.npy', 'wb'))
